...
function M.wrapper_script()
  ...
  return table.concat({
    ...
    '# a second editor. Flags, version-control message files, callers outside',
    '# muxim, and a session that no longer answers get the real nvim.',
    ...
    '"$client" --server "$NVIM" --remote-expr "1" >/dev/null 2>&1 || passthrough "$@"',
    'opened_dir=0',
    'for arg in "$@"; do',
    '  if [ -d "$arg" ]; then',
    '    opened_dir=1',
